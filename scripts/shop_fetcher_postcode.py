...
class ShopFetcherPostcode:
    ...
    def load_postcodes(self, postcode_file):
        if not os.path.exists(postcode_file):
            raise FileNotFoundError(f"Postcode file not found: {postcode_file}")
    ...
    def _fetch_and_save(self, postcode):
        proxy = self._get_random_proxy()
        ua = self.user_agent.random

        try:
            data = self.api.get_restaurants_by_postcode(postcode, user_agent=ua, proxy=proxy)
        except Exception as e:
            print(f"❌ Request error for postcode {postcode}: {e}")
            return
        ...
        if data:
            for shop in data:
                ...
                try:
                    ...
                    print(f"✅ Saved postcode {postcode}")
                except Exception as e:
                    print(f"❌ Error saving postcode {postcode}: {e}")
        
        else:
            print(f"❌ Empty or invalid for postcode {postcode}")
    ...
    async def run(self):
        ...
        postcodes = self.load_postcodes(self.postcode_file)
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            tasks = []
            for postcode in postcodes:
                task = loop.run_in_executor(executor, self._fetch_and_save, postcode)
                tasks.append(task)
...
if __name__ == "__main__":
    fetcher = ShopFetcherPostcode(
        postcode_file="config/postcodes.txt",
        use_proxy=False,
